...
// check if P96 is present and if we don't need the fix
	...
	if ( !IsCyberModeID(GetVPModeID(&SDL_Display->ViewPort)) ) {  //selden reach
		...
		if ( okid != INVALID_ID ) {  //selden reach

			GFX_Display = OpenScreenTags(NULL,
					//SA_Width,SDL_Display->Width,
					//SA_Height,SDL_Display->Height,
										 SA_Title, "include Screen",
										 SA_Depth, bpp, SA_Quiet, TRUE,
										 SA_ShowTitle, FALSE,
										 SA_DisplayID, okid,
										 TAG_DONE);
		} else
			D(bug("Cant get a valid screenmode ID with BestCModeIDTags\n"));
		...
		if ( flags & SDL_FULLSCREEN ) {
			...
			if ( left != 0 || top != 0 ) {
				SDL_Window_Background = OpenWindowTags(NULL, WA_Left, 0, WA_Top, 0,
						...
						/*|WFLG_REPORTMOUSE */,
													   WA_IDCMP, 0,
						//WA_Title,"include Backgroundwindow",
...
//								SA_Quiet,TRUE,SA_ShowTitle,FALSE,
					...
					D(bug("2: Cant get a valid screenmode ID with BestCModeIDTags\n"));
				if ( !GFX_Display) {
					GFX_Display = SDL_Display;
					flags &= ~SDL_FULLSCREEN;
					flags &= ~SDL_DOUBLEBUF;
...
static void CGX_VideoQuit(_THIS) {
	...
	if ( SDL_Display != NULL ) {
		...
		/* Unload include-GL library after X11 shuts down */
	}
